...
@section('content')
    ...
    <section class="grid grid-cols-1 gap-4 lg:grid-cols-[0.8fr_1.2fr]">
        <x-ui.card>
            <div class="mx-auto flex h-20 w-20 items-center justify-center rounded-3xl bg-linear-to-br from-cyan-500 to-teal-500 text-3xl font-semibold text-white">
                {{ strtoupper(substr($user->name, 0, 1)) }}
            </div>
            <p class="mt-3 text-center text-lg font-semibold">{{ $user->name }}</p>
            <p class="text-center text-sm text-slate-500 dark:text-slate-400">{{ $user->email }}</p>
